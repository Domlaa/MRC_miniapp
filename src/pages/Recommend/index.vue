<template>
	<View class="Recommend">
		<Swiper @change="swiperChange" :current="current" indicatorColor="#fff" indicatorActiveColor="#31c27c" circular
		 indicatorDots autoplay>
			<SwiperItem v-for="(item, index) in banner" :key="index">
				<Image lazyLoad style="" :src="
                        show.includes(index)
		...
		<View class="cell-SongSheet">
			<SongSheet v-for="(item, index) in PersonalizedList" :key="index" :Oid="item.id" :name="item.name" :picUrl="item.picUrl + '?imageView&thumbnail=250x0'"
